...
def get_mnist(args, data_dir='./data/koreatechno'):
    """get dataloders"""
...
#    test = datasets.MNIST(root=data_dir, train=False, download=True)
    ...
    train = tv.datasets.ImageFolder(root='./data/koreatechno/train')
    test = tv.datasets.ImageFolder(root='./data/koreatechno/test')
    
#    x_train = torch.FloatTensor(1649,3,480,640)
#    y_train = torch.Tensor(1649)
    
    x_train = [0 for i in range(1649)]
    y_train = [0 for i in range(1649)]
    
    for i in range(1649):
        x_train[i]=train[i][0]
            
#    x_test = torch.FloatTensor(35,3,480,640)
#    y_test = torch.Tensor(35)
        
    x_test = [0 for i in range(35)]
    y_test = [0 for i in range(35)]
